...
explore: sessions {
  ...
  join: forecasting {
    type: inner
    sql_on:${events.event_name}=${forecasting.events_event_name} OR ${events.event_time_date}=${forecasting.forecast_timestamp} ;; # ${events.event_name}=${forecasting.events_event_name} AND ${events.event_time_date}=${forecasting.forecast_timestamp} ;;
