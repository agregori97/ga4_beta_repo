  ...
  derived_table: {
    ...
    (WITH
      visitors_labeled AS (
        ...
        WHERE
          DATE(session_date) > FORMAT_DATE('%Y%m%d', DATE_SUB(CURRENT_DATE(), INTERVAL (@{GA4_BQML_train_months} + @{GA4_BQML_test_months}) MONTH)) AND DATE(session_date) < FORMAT_DATE('%Y%m%d', DATE_SUB(CURRENT_DATE(), INTERVAL @{GA4_BQML_test_months} MONTH))
        GROUP BY
          user_pseudo_id
      ),
      visitor_location AS (
