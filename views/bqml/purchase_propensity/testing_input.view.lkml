  ...
  derived_table: {
    ...
    (WITH
      visitors_labeled AS (
        ...
        WHERE
          session_date > FORMAT_DATE('%Y%m%d', DATE_SUB(CURRENT_DATE(), INTERVAL @{GA4_BQML_test_months} MONTH))
        GROUP BY
          user_pseudo_id
      ),
      visitor_location AS (
      ...
      user_model AS (
        ...
        WHERE
          session_date > TIMESTAMP(FORMAT_DATE('%Y%m%d', DATE_SUB(CURRENT_DATE(), INTERVAL @{GA4_BQML_test_months} MONTH)))
          AND (
            GA.event_timestamp < IFNULL(event_session, 0)
            OR event_session IS NULL)
          AND platform IN ('WEB')
